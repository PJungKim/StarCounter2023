...
- 화살표는 주행 방향을 나타내는 표시로, 실제 대회 참가자가 주행 방향을 잘못 보는 일이 없게 하는 장치이다.
- 화살표를 출력하기 전 화살표의 색을 정해야 하며, `(matrix_t).SetColor(색상)`의 형태로 사용한다.
- 화살표 함수는 `(matrix_t).Arrow(방향)`의 형태로 사용할 수 있다.
